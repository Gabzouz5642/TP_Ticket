...
Détails des Casts (Sécurité des types)Le système force la précision des données financières et temporelles via Eloquent :Heures : Casté en decimal:2 pour éviter les erreurs d'arrondi flottant.Authentification : Le mot de passe utilisateur est automatiquement haché via le cast hashed.

🔄 Logique Métier & FluxProcessus de création d'un TicketLe flux suivant décrit la validation et l'enregistrement d'une intervention :Extrait de codesequenceDiagram
...
Couverture actuelle : Tests unitaires des modèles et tests de fonctionnalités des routes principales.

📦 Scripts Utiles (Composer)composer run dev : Lance simultanément le serveur Laravel, Vite, et le processeur de files d'attente.composer run test : Nettoie la configuration et lance la suite de tests.
